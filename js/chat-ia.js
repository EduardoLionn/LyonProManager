...
Se não sugerir jogador nenhum nesta resposta, deixe "sugestoes" como um array vazio [].`;
                    ...
                    let entrantesSubStr = (partidaAtual && Array.isArray(partidaAtual.substituicoes) && partidaAtual.substituicoes.length)
                        ...
                        : [];
                    // Preparo físico de cada reserva vai junto pro prompt — sem isso a IA não tem
                    // como saber quem realmente precisa de minutos pra recuperar condição (pedido
                    // do treinador: ela sugeriu um jogador que JÁ estava com preparo físico bom
                    // quando o pedido era justamente ajudar alguém que precisava dele).
                    let condicaoTexto = (jogBD) => (jogBD && typeof nivelPreparoFisico === 'function' && typeof jogBD.preparoFisico === 'number')
                        ? ` | preparo físico: ${Math.round(jogBD.preparoFisico)}% (${nivelPreparoFisico(jogBD.preparoFisico).rotulo})`
                        : '';
                    let escalacaoStr = (partidaAtual && partidaAtual.titulares)
                        ? `Partida ${partidaAtual.status === 'em_andamento' ? 'EM ANDAMENTO' : 'declarada'} contra ${partidaAtual.adversarioNome}. Formação atual em campo: ${partidaAtual.formacaoEscolhida}. Titulares (sigla da posição no campo: nome, preparo físico): ${Object.entries(partidaAtual.titulares).map(([pos, j]) => { let jogBD = db[currentSave].plantel.find(p => p.nome === j.nome); return `${pos}: ${j.nome}${condicaoTexto(jogBD)}`; }).join(', ')}. Banco (nome, posição real, histórico de uso e preparo físico do jogador): ${(partidaAtual.banco || []).map(b => { let jogBD = db[currentSave].plantel.find(p => p.nome === b.nome); let posTxt = (jogBD ? jogBD.posicao : b.posicao) || 'posição desconhecida'; let habituaisStr = posicoesHabituaisTexto(b.nome); return `${b.nome} (${posTxt}${habituaisStr ? ' | costuma jogar: ' + habituaisStr : ''}${condicaoTexto(jogBD)})`; }).join(', ') || 'nenhum registrado'}.${(partidaAtual.jogadoresForaDaPartida && partidaAtual.jogadoresForaDaPartida.length > 0) ? ` JÁ SAÍRAM DA PARTIDA E NÃO PODEM MAIS ENTRAR: ${partidaAtual.jogadoresForaDaPartida.join(', ')}.` : ''}${entrantesSubStr.length ? ` JÁ ENTRARAM COMO SUBSTITUTOS NESTA PARTIDA (NUNCA escolha nenhum destes como "jogadorSai" — só saem por lesão real, declarada pelo treinador): ${entrantesSubStr.join(', ')}.` : ''}`
        ...
        - SE o treinador pedir EXPLICITAMENTE uma troca por causa de PREPARO FÍSICO (ex: "coloca alguém pra dar mais preparo físico", "precisa descansar gente"), o objetivo é dar minutos pra quem MAIS PRECISA — escolha como "jogadorEntra" o reserva posicionalmente compatível com o PREPARO FÍSICO MAIS BAIXO entre os candidatos (veja o preparo físico de cada um listado no Banco acima), nunca alguém que já esteja com preparo físico bom/alto — colocar quem já está bem preparado não atende esse pedido.
        ...
        🔄 MUDANÇA DE FORMAÇÃO (USE COM MUITA RESERVA): você também pode sugerir mudar a formação inteira do time (não gasta substituição, os mesmos 11 jogadores em campo só mudam de esquema) — mas isso é uma decisão GRANDE, não um ajuste fino. SÓ preencha "sugestaoFormacao" se o contexto (texto e/ou imagem) descrever uma situação EXTREMA, como: (a) uma GOLEADA em andamento (diferença de 3 gols ou mais, pra qualquer lado) ou (b) DOMÍNIO ABSURDO do adversário (ex: adversário controlando o jogo do início ao fim, posse esmagadora somada a múltiplas chances claras, time completamente incapaz de sair jogando). Uma dificuldade pontual, uma fase ruim de 10 minutos, ou "estamos sendo um pouco pressionados" NÃO justificam mudar a formação — nesses casos oriente taticamente ou sugira uma substituição pontual, mas deixe "sugestaoFormacao" como null. Quando a situação for mesmo extrema, escolha uma formação DIFERENTE da atual (${partidaAtual.formacaoEscolhida}), EXATAMENTE entre as formações preferidas do treinador: ${formacoesPreferidasChat.join(', ')}. Nunca sugira uma formação fora dessa lista.` : '';
