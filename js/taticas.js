...
// -------------------------------------------------------------------------------------
// INTERFACE — PAINEL DO PERFIL DO TREINADOR
// -------------------------------------------------------------------------------------

function _optionsFormacoes(selecionada) {
    return listaFormacoesDisponiveis()
        .map(f => `<option value="${f}"${f === selecionada ? ' selected' : ''}>${f}</option>`)
        .join('');
}

function renderizarPainelTaticas() {
    let container = document.getElementById('painel-taticas');
    ...
    // Cartões de predefinição — o que estiver travando outro campo aparece marcado
    let cardsPre = PREDEFINICOES_TATICAS.map(p => `
        <button type="button" class="tatica-card${p.id === t.predefinicao ? ' tatica-card-ativo' : ''}" onclick="escolherPredefinicao('${p.id}')">
            <span class="tatica-card-nome">${p.emoji} ${p.nome}</span>
            <span class="tatica-card-desc">${p.descricao}</span>
            <span class="tatica-card-regra">${textoRegraPredefinicao(p.id)}</span>
        </button>`).join('');

    // Estilos de armação: o proibido/obrigatório pela predefinição fica desabilitado
    let cardsEstilo = ESTILOS_ARMACAO.map(e => {
        let bloqueado = (regra.estiloObrigatorio && regra.estiloObrigatorio !== e.id)
                     || (regra.estilosProibidos && regra.estilosProibidos.includes(e.id));
        return `
        <button type="button" class="tatica-card tatica-card-menor${e.id === t.estiloArmacao ? ' tatica-card-ativo' : ''}${bloqueado ? ' tatica-card-bloqueado' : ''}"
                ${bloqueado ? 'disabled title="Bloqueado pela predefinição tática atual"' : `onclick="escolherEstiloArmacao('${e.id}')"`}>
            <span class="tatica-card-nome">${e.nome}${bloqueado ? ' 🔒' : ''}</span>
            <span class="tatica-card-desc">${e.descricao}</span>
        </button>`;
    }).join('');

    // Faixas da abordagem defensiva, com as bloqueadas marcadas
    let faixasHtml = ABORDAGENS_DEFENSIVAS.map(a => {
        let permitida = !regra.abordagensPermitidas || regra.abordagensPermitidas.includes(a.id);
        let ativa = a.id === faixa.id;
        return `<div class="tatica-faixa${ativa ? ' tatica-faixa-ativa' : ''}${permitida ? '' : ' tatica-faixa-bloqueada'}" style="${ativa ? `border-color:${a.cor}; color:${a.cor};` : ''}">
            <strong>${a.nome}${permitida ? '' : ' 🔒'}</strong>
            <span>${a.min}–${a.max}</span>
        </div>`;
    }).join('');

    container.innerHTML = `
        <div class="tatica-bloco">
            <h3 class="tatica-titulo">1. Predefinição Tática</h3>
            <p class="tatica-ajuda">O pacote que você seleciona no jogo. Cada um trava uma das escolhas abaixo — a trava aparece no próprio cartão.</p>
            <div class="tatica-grade">${cardsPre}</div>
        </div>

        <div class="tatica-bloco">
            <h3 class="tatica-titulo">2. Esquema (Formação)</h3>
            <p class="tatica-ajuda">O esquema principal e um alternativo, que o Auxiliar usa como plano B quando o adversário pede outra postura.</p>
            <div class="grid-2">
                <div class="linha-form">
                    <label style="color:var(--primary);">Esquema principal</label>
                    <select id="tatica-esquema" onchange="escolherEsquema('esquema', this.value)" style="background: var(--bg-dark);">${_optionsFormacoes(t.esquema)}</select>
                </div>
                <div class="linha-form">
                    <label style="color:var(--primary);">Esquema alternativo</label>
                    <select id="tatica-esquema-alt" onchange="escolherEsquema('esquemaAlternativo', this.value)" style="background: var(--bg-dark);">${_optionsFormacoes(t.esquemaAlternativo)}</select>
                </div>
            </div>
        </div>

        <div class="tatica-bloco">
            <h3 class="tatica-titulo">3. Estilo de Armação</h3>
            <p class="tatica-ajuda">Como a bola sai de trás.</p>
            <div class="tatica-grade">${cardsEstilo}</div>
        </div>

        <div class="tatica-bloco">
            <h3 class="tatica-titulo">4. Abordagem Defensiva</h3>
            <p class="tatica-ajuda">Altura da linha defensiva, de 0 a 100 — o mesmo número que aparece no jogo.</p>
            <div class="tatica-slider-wrap">
                <input type="range" min="0" max="100" step="1" id="tatica-abordagem" value="${t.abordagemDefensiva}"
                       oninput="previewAbordagem(this.value)" onchange="escolherAbordagem(this.value)" class="tatica-slider">
                <div class="tatica-slider-valor" id="tatica-abordagem-valor" style="color:${faixa.cor};">
                    <strong>${t.abordagemDefensiva}</strong> — ${faixa.nome}
                </div>
            </div>
            <div class="tatica-faixas">${faixasHtml}</div>
            <p class="tatica-ajuda" id="tatica-abordagem-desc" style="margin-top:10px;">${faixa.descricao}</p>
        </div>

        <div class="tatica-resumo">
            <strong>📋 Como aplicar no jogo:</strong>
            <span>${pre.emoji} ${pre.nome} &nbsp;•&nbsp; ${t.esquema} &nbsp;•&nbsp; ${est.nome} &nbsp;•&nbsp; ${t.abordagemDefensiva}/100 (${faixa.nome})</span>
        </div>
    `;
}

// Cada handler aplica a escolha, deixa o motor de regras corrigir o resto e avisa o que mudou.
function _aplicarEscolhaTatica(mudanca) {
    let atual = Object.assign({}, taticaDoSave(), mudanca);
    let resultado = salvarTaticaNoSave(atual);
    renderizarPainelTaticas();
    if (resultado && resultado.ajustes.length) {
        let aviso = document.getElementById('tatica-aviso-regra');
        if (aviso) {
            aviso.style.display = 'block';
            aviso.innerHTML = `⚠️ <strong>Ajuste automático (regra do jogo):</strong><br>${resultado.ajustes.join('<br>')}`;
            clearTimeout(window._timeoutAvisoTatica);
            window._timeoutAvisoTatica = setTimeout(() => { aviso.style.display = 'none'; }, 9000);
        }
    }
}

function escolherPredefinicao(id) { _aplicarEscolhaTatica({ predefinicao: id }); }
function escolherEstiloArmacao(id) { _aplicarEscolhaTatica({ estiloArmacao: id }); }
function escolherEsquema(campo, valor) { _aplicarEscolhaTatica({ [campo]: valor }); }
function escolherAbordagem(valor) { _aplicarEscolhaTatica({ abordagemDefensiva: Number(valor) }); }

// Enquanto arrasta o slider, atualiza só o rótulo (sem gravar a cada pixel)
function previewAbordagem(valor) {
    let faixa = faixaAbordagem(valor);
    let el = document.getElementById('tatica-abordagem-valor');
    let desc = document.getElementById('tatica-abordagem-desc');
    if (el) { el.innerHTML = `<strong>${valor}</strong> — ${faixa.nome}`; el.style.color = faixa.cor; }
    if (desc) desc.innerText = faixa.descricao;
}
